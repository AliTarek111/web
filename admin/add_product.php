...
// Ahmed Koshary Store - Professional Product Creation Hub
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        // 1. Insert Product
        $stmt = $pdo->prepare("INSERT INTO products (uid, name, price, category_id, stock_count, condition_status, description, main_image, product_type, software_version, battery_health, is_featured) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$uid, $name, $price, $category_id, $stock, $condition, $desc, $image, $type, $version, $battery, $is_featured]);
        $productId = $pdo->lastInsertId();

        // 2. Insert Specifications
        if (isset($_POST['spec_keys']) && isset($_POST['spec_values'])) {
            $spec_stmt = $pdo->prepare("INSERT INTO product_specs (product_id, spec_key, spec_value) VALUES (?, ?, ?)");
            for ($i = 0; $i < count($_POST['spec_keys']); $i++) {
                $key = trim($_POST['spec_keys'][$i]);
                $val = trim($_POST['spec_values'][$i]);
                if (!empty($key) && !empty($val)) {
                    $spec_stmt->execute([$productId, $key, $val]);
                }
            }
        }

        $pdo->commit();
        $message = "تم إضافة المنتج والمواصفات بنجاح!";
...
<header class="mb-16 flex flex-col md:flex-row justify-between items-start md:items-end gap-6 relative z-10">
    <div>
        ...
        <h1 class="text-5xl font-black font-headline text-on-surface tracking-tighter uppercase leading-tight">إضافة <br/><span class="bg-gradient-to-l from-primary to-primary-container bg-clip-text text-transparent">منتج جديد للمتجر</span></h1>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div class="lg:col-span-2 space-y-8">
            <section class="glass-card rounded-[2.5rem] p-10 border border-white/5 shadow-2xl space-y-10">
                <h3 class="text-sm font-black uppercase tracking-widest text-primary/60 border-b border-primary/10 pb-4">المعلومات الأساسية</h3>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-3">
                        <label class="text-[9px] font-black uppercase tracking-[0.3em] text-primary/40 pr-2">اسم المنتج</label>
                        <input required name="name" type="text" placeholder="iPhone 15 Pro Max" class="w-full bg-[#0a0e18] border-white/5 rounded-2xl focus:border-primary text-sm p-5 text-on-surface transition-all"/>
                    </div>
                    <div class="space-y-3">
                        <label class="text-[9px] font-black uppercase tracking-[0.3em] text-primary/40 pr-2">كود المنتج</label>
                        <input required name="uid" type="text" placeholder="BOSS-99XX" class="w-full bg-[#0a0e18] border-white/5 rounded-2xl focus:border-primary text-sm p-5 text-on-surface transition-all"/>
                    </div>
                    <div class="space-y-3">
                        <label class="text-[9px] font-black uppercase tracking-[0.3em] text-primary/40 pr-2">السعر (ج.م)</label>
                        <input required name="price" type="number" step="0.01" class="w-full bg-[#0a0e18] border-white/5 rounded-2xl focus:border-primary text-sm p-5 text-on-surface transition-all font-bold"/>
                    </div>
                    <div class="space-y-3">
                        <label class="text-[9px] font-black uppercase tracking-[0.3em] text-primary/40 pr-2">الكمية المتوفرة</label>
                        <input required name="stock" type="number" class="w-full bg-[#0a0e18] border-white/5 rounded-2xl focus:border-primary text-sm p-5 text-on-surface transition-all"/>
                    </div>
                </div>

                <div class="space-y-3">
                    <label class="text-[9px] font-black uppercase tracking-[0.3em] text-primary/40 pr-2">الوصف التفصيلي</label>
                    <textarea name="description" rows="6" class="w-full bg-[#0a0e18] border-white/5 rounded-3xl focus:border-primary text-sm p-6 text-on-surface transition-all"></textarea>
                </div>
            </section>
            ...
            <section class="glass-card rounded-[2.5rem] p-10 border border-white/5 shadow-2xl space-y-10">
                ...
                <div id="specsContainer" class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 items-end animate-fade-in">
                        <div class="space-y-2">
                            <label class="text-[8px] font-black uppercase tracking-widest text-on-surface-variant/20 pr-2">الخاصية (مثل: المعالج)</label>
                            <input name="spec_keys[]" type="text" placeholder="Processor" class="w-full bg-[#0a0e18]/50 border-white/5 rounded-xl text-xs p-4 text-on-surface"/>
                        </div>
                        <div class="flex gap-4 items-end">
                            <div class="space-y-2 flex-1">
                                <label class="text-[8px] font-black uppercase tracking-widest text-on-surface-variant/20 pr-2">القيمة (مثل: Apple A17)</label>
                                <input name="spec_values[]" type="text" placeholder="Bionic Quantum" class="w-full bg-[#0a0e18]/50 border-white/5 rounded-xl text-xs p-4 text-on-surface"/>
                            </div>
        ...
        <div class="space-y-8">
            <section class="glass-card rounded-[2.5rem] p-10 border border-white/5 shadow-2xl space-y-8">
                <h3 class="text-sm font-black uppercase tracking-widest text-primary/60 border-b border-primary/10 pb-4">التحكم في النشر</h3>
                
                <div class="space-y-6">
                    <div class="space-y-3">
                        <label class="text-[9px] font-black uppercase tracking-[0.3em] text-primary/40 pr-2">القسم</label>
                    ...
                    <div class="space-y-3">
                        <label class="text-[9px] font-black uppercase tracking-[0.3em] text-primary/40 pr-2">نوع المنتج</label>
                        <div class="flex gap-4">
                            <label class="flex-1 cursor-pointer group">
                                <input type="radio" name="product_type" value="hardware" checked class="hidden peer">
                                <div class="p-4 rounded-2xl border border-white/5 bg-[#0a0e18] text-center text-[10px] font-black uppercase tracking-widest text-on-surface-variant/40 peer-checked:border-primary peer-checked:text-primary transition-all group-hover:bg-white/5">Hardware</div>
                            </label>
                            <label class="flex-1 cursor-pointer group">
                                <input type="radio" name="product_type" value="software" class="hidden peer">
                                <div class="p-4 rounded-2xl border border-white/5 bg-[#0a0e18] text-center text-[10px] font-black uppercase tracking-widest text-on-surface-variant/40 peer-checked:border-primary peer-checked:text-primary transition-all group-hover:bg-white/5">Software</div>
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="space-y-3" id="condition_container">
                        ...
                        <select name="condition" class="w-full bg-[#0a0e18] border-white/5 rounded-2xl focus:border-primary text-sm p-4 text-on-surface transition-all cursor-pointer">
                            <option value="new">جديد (New Sovereign)</option>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-2" id="battery_container">
                            ...
                            <input name="battery_health" type="number" value="100" class="w-full bg-[#0a0e18] border-white/5 rounded-xl text-xs p-3 text-on-surface"/>
                        </div>
                        <div class="space-y-2" id="software_container" style="display: none;">
                            ...
                            <input name="software_version" type="text" placeholder="iOS 17.4" class="w-full bg-[#0a0e18] border-white/5 rounded-xl text-xs p-3 text-on-surface"/>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section class="glass-card rounded-[2.5rem] p-10 border border-white/5 shadow-2xl space-y-6">
                ...
                <div class="relative w-full aspect-square border-2 border-dashed border-white/5 rounded-[2rem] hover:border-primary/40 transition-all flex flex-col items-center justify-center gap-4 group cursor-pointer overflow-hidden bg-[#0a0e18]">
                    <span class="material-symbols-outlined text-4xl text-white/5 absolute group-hover:text-primary/20 group-hover:scale-125 transition-all">image</span>
                    <input type="file" name="main_image" class="absolute inset-0 opacity-0 cursor-pointer z-10" onchange="previewImage(this)"/>
                    <img id="imagePreview" src="" class="hidden absolute inset-0 w-full h-full object-contain p-4"/>
                    <p id="uploadLabel" class="text-[8px] text-on-surface-variant/20 font-black uppercase tracking-widest relative z-0">انقر لرفع صورة المنتج</p>
                </div>
            </section>
