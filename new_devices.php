<?php
// new_devices.php
// Ahmed Koshary Store - New Devices
include 'includes/db.php';

$query = "SELECT p.*, c.name as category_name 
          FROM products p 
          LEFT JOIN categories c ON p.category_id = c.id 
          WHERE p.is_active = 1 AND p.condition_status = 'new' AND (c.slug = 'smartphones' OR c.name LIKE '%موبيل%' OR c.name LIKE '%هواتف%')
          ORDER BY p.created_at DESC";
$products = $pdo->query($query)->fetchAll();

$pageTitle = "الأجهزة الجديدة";
...
<section class="pt-32 pb-16 bg-surface-container-lowest relative overflow-hidden">
    ...
    <div class="container mx-auto px-6 relative z-10 text-center">
        <h1 class="text-5xl md:text-6xl font-black font-headline mb-6 text-on-surface">الأجهزة <span class="text-primary bg-gradient-to-r from-primary to-primary/60 bg-clip-text">الجديدة (New)</span></h1>
        <p class="text-on-surface-variant/70 text-lg max-w-2xl mx-auto">تصفح أحدث الهواتف الذكية بحالتها الأصلية مع ضمان معتمد وراحة بال تامة.</p>
    </div>
</section>
            ...
            <?php if (empty($products)): ?>
                <div class="col-span-full p-20 text-center glass-card rounded-[2.5rem] border border-white/5 flex flex-col items-center justify-center gap-6">
                    <div class="w-20 h-20 bg-white/5 rounded-full flex items-center justify-center">
                        <span class="material-symbols-outlined text-4xl text-on-surface-variant/40">new_releases</span>
                    </div>
                    <p class="text-on-surface-variant/60 font-bold uppercase tracking-widest text-sm">لا توجد أجهزة جديدة متوفرة حالياً.</p>
                </div>
            <?php else: ?>
